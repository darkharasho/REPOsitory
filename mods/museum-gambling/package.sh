...
DLL="bin/Release/netstandard2.1/MuseumGambling.dll"
BUILDS_DIR="$(cd "$(dirname "$0")/../.." && pwd)/builds"
mkdir -p "$BUILDS_DIR"
OUT="$BUILDS_DIR/MuseumGambling-${VERSION}.zip"
...
dotnet build MuseumGambling.csproj --configuration Release /p:GameDir="$GAME_DIR"
