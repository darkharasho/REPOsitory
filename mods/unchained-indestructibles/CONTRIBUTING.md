...
**Windows:**
```bash
dotnet build UnchainedIndestructibles.csproj --configuration Release
# Override game path if not the default Steam location:
dotnet build UnchainedIndestructibles.csproj --configuration Release /p:GameDir="D:\Games\REPO"
```
...
Output DLL: `bin/Release/netstandard2.1/UnchainedIndestructibles.dll`
...
Outputs `UnchainedIndestructibles-<version>.zip` ready for Thunderstore upload.

**Manual zip** (no subdirectory):
- `manifest.json`
- `icon.png`
- `README.md`
- `bin/Release/netstandard2.1/UnchainedIndestructibles.dll`
