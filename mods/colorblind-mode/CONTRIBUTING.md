...
**Windows:**
```bash
dotnet build ColorblindMode.csproj --configuration Release
# Override game path if not the default Steam location:
dotnet build ColorblindMode.csproj --configuration Release /p:GameDir="D:\Games\REPO"
```
...
Output DLL: `bin/Release/netstandard2.1/ColorblindMode.dll`
...
Outputs `ColorblindMode-<version>.zip` ready for Thunderstore upload.

**Manual zip** (no subdirectory):
- `manifest.json`
- `icon.png`
- `README.md`
- `bin/Release/netstandard2.1/ColorblindMode.dll`
