...
DLL="bin/Release/netstandard2.1/ColorblindMode.dll"
BUILDS_DIR="$(cd "$(dirname "$0")/../.." && pwd)/builds"
mkdir -p "$BUILDS_DIR"
OUT="$BUILDS_DIR/ColorblindMode-${VERSION}.zip"
...
dotnet build ColorblindMode.csproj --configuration Release /p:GameDir="$GAME_DIR"
