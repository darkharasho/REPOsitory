...
DLL="bin/Release/netstandard2.1/ForcedFriendship.dll"
BUILDS_DIR="$(cd "$(dirname "$0")/../.." && pwd)/builds"
mkdir -p "$BUILDS_DIR"
OUT="$BUILDS_DIR/ForcedFriendship-${VERSION}.zip"
...
dotnet build ForcedFriendship.csproj --configuration Release /p:GameDir="$GAME_DIR"
